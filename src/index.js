// source: https://github.com/danburzo/percollate/blob/master/index.js (© Dan Burzo)
// the original code is licensed under MIT license
// the original code is modified to fit the needs of this project
...
} from "percollate/src/constants/markdown.js";

import slurp from "percollate/src/util/slurp.js";
import fileMimetype from "percollate/src/util/file-mimetype.js";
import epubDate from "percollate/src/util/epub-date.js";
import humanDate from "percollate/src/util/human-date.js";
import outputPath from "percollate/src/util/output-path.js";
import getCssPageFormat from "percollate/src/util/get-css-page-format.js";
import {
  resolveSequence,
  resolveParallel,
} from "percollate/src/util/promises.js";
import addExif from "percollate/src/exif.js";
import { hyphenateDom } from "percollate/src/hyphenate.js";
import {
  textToIso6391,
  getLanguageAttribute,
} from "percollate/src/util/language.js";
...
import {
  ...
  githubSpecific,
  wrapPreBlocks,
} from "percollate/src/enhancements.js";
import mapRemoteResources from "percollate/src/remote-resources.js";
import inlineImages from "percollate/src/inline-images.js";
import get_style_attribute_value from "percollate/src/get-style-attribute-value.js";
...
const pkg = JSON.parse(
  fs.readFileSync(
    new URL("../node_modules/percollate/package.json", import.meta.url)
  )
);
...
async function cleanup(url, options) {
  ...
  try {
    ...
    const lang =
      getLanguageAttribute(dom.window.document) || textToIso6391(textContent);
...
async function bundlePdf(items, options) {
  const DEFAULT_STYLESHEET = new URL("templates/default.css", import.meta.url);
  ...
  const html = nunjucks.renderString(
    await readFile(options.template || DEFAULT_TEMPLATE, "utf8"),
    {
      ...
      style,
      options: {
        use_toc: options.toc || (items.length > 1 && options.toc !== false),
        use_cover:
          options.cover ||
          (options.cover !== false && (options.title || items.length > 1)),
      },
    }
  );
...
async function epubgen(data, output_path, options) {
  ...
  return wrapAsync(async (resolve, reject) => {
    ...
    if (data.cover) {
      const COVER_TEMPLATE = new URL("templates/cover.html", import.meta.url);
